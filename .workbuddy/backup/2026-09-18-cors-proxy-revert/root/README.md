...
- **私有网关连不上？** 自建网关通常不放行 CORS（`OPTIONS` 预检 403），
  浏览器会直接抛 `Failed to fetch`，此限制**无法在前端绕过**。仓库自带零依赖本地代理，
  且 **`npm run dev` 会顺手把它一起起起来**（端口被占用则视为「已有代理在跑」，直接放行）：
  ```bash
  npm run dev           # 同时启动站点与本地代理（http://127.0.0.1:8787）
  npm run llm:proxy     # 也可以单独常驻启动
  ```
  再在「模型连接 → 高级：跨域代理与私有网关」点「填入本地代理」即可。
  代理前缀**漏写 `http://`**（如 `127.0.0.1:8787`）会自动补全 —— 这类写法过去会拼出
  非法地址、静默失败；请求失败时平台还会**自动探测代理**并给出确定结论，而不是只回一句
  「网络请求失败」。
...
### Optional: local CORS proxy for self-hosted LLM gateways

浏览器直连大模型接口需要对方放行 CORS。自建网关（One-API / new-api / 自研反代）
通常**不放行** —— `OPTIONS` 预检被 403 拒绝，浏览器直接抛 `Failed to fetch`，
且该失败发生在响应到达 JS 之前，前端无法捕获、也无法绕过。
仓库自带一个零依赖本地代理。**`npm run dev` 已默认把它一起启动**（用
`LLM_PROXY_DISABLE=true` 可关掉这段自动启动），也可以单独常驻：

```bash
npm run llm:proxy                     # 监听 http://127.0.0.1:8787
PORT=9000 npm run llm:proxy           # 换端口
LLM_PROXY_ALLOW=api.example.com npm run llm:proxy   # 限定上游域名白名单
```

然后在「模型连接 → 高级：跨域代理与私有网关」点「填入本地代理」。
只绑定 `127.0.0.1`、不保存任何 Key；SSE 逐段直通，流式输出不受影响。

两处防呆设计（都来自真实事故）：

- **前缀自动归一化**：`127.0.0.1:8787`、`localhost:8787` 这类漏写协议或会被解析成
  非法 scheme 的写法统一补 `http://`；误带路径（`…:8787/llm`）会当场提示。
  这些写法原本都会让请求静默失败成 `Failed to fetch`，报错完全不指向原因。
- **失败自动诊断**：请求以网络类错误失败且已配代理时，平台自动探测代理健康检查，
  直接给出「代理没启动」/「代理正常，问题在 Key 或接口地址」的确定结论。

一键复验：`node verify-llm-proxy.mjs`。
详见 [LLM Integration Guide](docs/llm-integration.md)。
...
├── scripts/              # Build-time compilers (catalogue, learning content) + local LLM CORS proxy
...
| [LLM Integration Guide](docs/llm-integration.md) | How the LLM-grounded retrieval Q&A works, supported providers (GPT / DeepSeek / GLM / Qwen / Kimi / SiliconFlow / Ollama), CORS findings, the built-in local CORS proxy (`npm run llm:proxy`) for self-hosted gateways, and security notes |
