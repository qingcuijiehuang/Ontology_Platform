...
/** 无法解析时的兜底输出上限（token）。 */
export const DEFAULT_OUTPUT_TOKENS = 1200;
...
export function sanitizeLlmConfig(config: Partial<LlmConfig> | null | undefined): LlmConfig {
  ...
  return {
    ...
    maxTokens: normalizeMaxTokens(merged.maxTokens),
...
/**
 * 本项目内置的零依赖本地 CORS 代理默认地址（scripts/llm-proxy.mjs）。
 * 用 127.0.0.1 而不是 localhost：避免个别 Windows 环境把 localhost 解析成 IPv6
 * `::1` 而代理只监听 IPv4 导致「明明起来了却连不上」。
 * 与 scripts/llm-proxy.mjs 的 DEFAULT_HOST / DEFAULT_PORT 保持一致（有测试锁定）。
 */
export const DEFAULT_LOCAL_PROXY_BASE = 'http://127.0.0.1:8787';

/**
 * 归一化代理前缀。
 *
 * 这个字段是**手填**的，而两种最常见的写法都会让请求**静默地**变成
 * `TypeError: Failed to fetch` —— 报错信息完全不指向真实原因，用户只会以为
 * 「代理没起作用」：
 *   - `127.0.0.1:8787` → 拼出来不是绝对 URL，`fetch` 直接抛 Invalid URL
 *   - `localhost:8787` → 被 URL 解析器当成 scheme 为 `localhost:` 的地址，
 *     而 fetch 只接受 http/https
 * 因此这里统一补 `http://`（内置代理就是 http，且只监听回环地址）。
 * 非 http(s) 的显式协议（如 `ftp://host`）原样保留，交给 proxyPrefixIssue 报错。
 */
export function normalizeProxyBase(proxyBaseUrl?: string): string {
  // 顺手去掉内部空白：从聊天窗口/文档里复制地址时常夹带空格或换行
  const raw = (proxyBaseUrl ?? '').trim().replace(/\s+/g, '');
  if (!raw) return '';
  // 已是 `scheme://xxx` 形式 → 只裁掉尾部斜杠
  if (/^[a-z][a-z0-9+.-]*:\/\//i.test(raw)) return raw.replace(/\/+$/, '');
  // 其余一律补 http://；`//host:port` 这种协议相对写法先去前导斜杠
  return `http://${raw.replace(/^\/+/, '')}`.replace(/\/+$/, '');
}

/**
 * 校验「代理前缀」的写法，返回可直接展示的中文问题描述；写法没问题时返回 null。
 *
 * 只做**写法**层面的校验（协议、路径），不探测服务是否真的在跑 —— 后者是
 * `probeProxy` 的职责。两类问题必须分开，因为修法完全不同。
 */
export function proxyPrefixIssue(proxyBaseUrl?: string): string | null {
  ...
  try {
    ...
    return `「${raw}」不是合法地址，请按 http://主机:端口 的格式填写，例如 ${DEFAULT_LOCAL_PROXY_BASE}。`;
  }
  if (url.protocol !== 'http:' && url.protocol !== 'https:') {
    return `代理前缀只支持 http:// 或 https://，当前写的是 ${url.protocol}//。请改成 ${DEFAULT_LOCAL_PROXY_BASE} 这样的形式。`;
  }
  if (url.pathname !== '/') {
    // 平台会自己把接口地址整段拼在代理前缀后面，前缀再多一层路径会拼到代理上不存在的路由
    return `代理前缀不要带路径（当前带了 ${url.pathname}），只填到端口即可，例如 ${DEFAULT_LOCAL_PROXY_BASE}。`;
  }
  return null;
}
...
export function withProxy(endpoint: string, proxyBaseUrl?: string): string {
  const proxy = normalizeProxyBase(proxyBaseUrl);
