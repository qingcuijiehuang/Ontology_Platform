...
export function AIQueryConsole({ onOpenModelSettings }: AIQueryConsoleProps) {
  ...
  const handleAsk = useCallback(async (rawQuestion?: string) => {
    ...
    try {
      ...
      const paramError = code === 'bad-request' || isOutputLimitError(detail);
      const hasProxy = Boolean(llm.proxyBaseUrl?.trim());
      // 网络类失败：浏览器只抛一句 Failed to fetch，状态码与报错原文都拿不到，
      // 光看这句话无法区分「代理没起 / 前缀写错 / Key 不对」。这里自动探一次代理，
      // 直接把确定结论写进提示 —— 比让用户自己去点「检测代理」猜一次可靠。
      let proxyHint: string | null = null;
      if ((code === 'cors' || code === 'network') && hasProxy) {
        proxyHint = (await diagnoseProxyFailure(llm))?.message ?? null;
        // 探测是异步的，期间可能已发起新一轮提问或用户点了清空
        if (runIdRef.current !== runId) return;
      }
      setError({
        message: detail,
        hint: paramError
          ...
          : (proxyHint ?? llmErrorHint(code, { hasProxy })),
        paramError,
      });
